...
static void _process_packet(pbl_transport_packet *pkt)
{
    ...
    if(pkt->endpoint == ENDPOINT_FIRMWARE_VERSION)
    {       
        process_version_packet(pkt->data);
    }
    else if (pkt->endpoint == ENDPOINT_SET_TIME)
    {
        process_set_time_packet(pkt->data);
    }
    else if (pkt->endpoint == ENDPOINT_PHONE_MSG)
    {
...
{  
     ...
     */
    hw_bluetooth_init();
    /* We are blocked here while bluetooth further deletegates a runloop */
