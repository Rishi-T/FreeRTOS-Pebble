...
// const char *app_name = "colour";

static void colour_update_proc(Layer *layer, GContext *nGContext);
void colour_main(void);
void colour_init(void);
void colour_deinit(void);
void colour_tick(struct tm *tick_time, TimeUnits tick_units);
...
static void colour_window_load(Window *window)
{
    Layer *window_layer = window_get_root_layer(s_main_window);
    GRect bounds = layer_get_unobstructed_bounds(window_layer);

    s_canvas_layer = layer_create(bounds);
    layer_set_update_proc(s_canvas_layer, colour_update_proc);
    ...
    tick_timer_service_subscribe(SECOND_UNIT, colour_tick);
    layer_mark_dirty(s_canvas_layer);
    printf("WF load done\n");
}


static void colour_window_unload(Window *window)
{
...
void colour_init(void)
{
    ...
    window_set_window_handlers(s_main_window, (WindowHandlers) {
        .load = colour_window_load,
        .unload = colour_window_unload,
    });
